...
# 1. Inject playTing and showSendingMoneyAnimation into UI
if "const UI = {" in js:
    inject = """
        playTing() {
            try {
    ...
    js = js.replace("const UI = {", "const UI = {" + inject)

# 2. Skip Splash Screen
if "input.classList.add('success-pulse');" in js:
    js = js.replace("""input.classList.add('success-pulse');
                setTimeout(() => { App.Story.start(); App.Core.refresh(); }, 500);
            } else {""", """input.classList.add('success-pulse');
                setTimeout(() => { Auth.navTo('screen-app'); App.Core.refresh(); }, 500);
            } else {""")

# 3. Transactions Audio
if "UI.toast(type === 'income' ? 'Pemasukan berhasil dicatat' : 'Pengeluaran berhasil dicatat');" in js:
    js = js.replace("UI.toast(type === 'income' ? 'Pemasukan berhasil dicatat' : 'Pengeluaran berhasil dicatat');",
                    "if(type === 'income') { UI.showSendingMoneyAnimation('Memproses Pemasukan...', () => { UI.playTing(); UI.toast('Pemasukan berhasil dicatat'); }); } else { UI.playTing(); UI.toast('Pengeluaran berhasil dicatat'); }")

if "UI.toast('Mutasi berhasil!');" in js:
    js = js.replace("UI.toast('Mutasi berhasil!');", "UI.showSendingMoneyAnimation('Mengirim Uang...', () => { UI.playTing(); UI.toast('Mutasi berhasil!'); });")

# 4. Kategori Donut Tooltip
if "return ` ${ctx.label}: Rp ${ctx.raw.toLocaleString('id-ID')}`" in js:
...
if "listEl.innerHTML = catHtml;" in js:
    js = re.sub(r"let catHtml = '';.*?listEl\.innerHTML = catHtml;", "listEl.innerHTML = '';", js, flags=re.DOTALL)

# 5. Export PDF and Excel Native
if "const Export = {" in js:
    export_logic = """
    const Export = {
        async exportPDF(e) {
            e.preventDefault();
...
"""
    js = re.sub(r"const Export = \{.*?\n    \};\n", lambda m: export_logic + "\n", js, flags=re.DOTALL)

# 6. History Report Daily Chart
hist_injection = """
            ...
            charts.profileHist = new Chart(canvas, {
                ...
                data: {
                    ...
                    datasets: [{ data: Object.values(catMap), backgroundColor: ['#ef4444', '#f59e0b', '#10b981', '#3b82f6', '#8b5cf6', '#ec4899', '#14b8a6', '#f97316'], borderWidth:0 }]
                },
                options: {
...
"""
if "App.Analytics.createChart('chart-profile-hist'" in js:
    js = re.sub(r"App\.Analytics\.createChart\('chart-profile-hist'.*?datalabels: \{ display: false \} \} \}\);", lambda m: hist_injection, js, flags=re.DOTALL)


# 7. Wallet Insights Layout (Mahasiswa Vertically Stacked) and Dashboard Percent
wallet_logic = """
            const hList = await Data.get('history');
            const now = new Date();
...
if "const ratioTunai = total > 0 ?" in js:
    js = re.sub(r"const ratioTunai = total > 0 \? \(w\.tunai \/ total\) \* 100 : 0;.*?document\.getElementById\('wallet-ratio-tunai'\)\.textContent = `\$\{Math\.round\(ratioTunai\)\}%`; \}", lambda m: wallet_logic, js, flags=re.DOTALL)

if "if (!b) { info.textContent =" in js:
    js = re.sub(r"if \(!b\) \{ info\.textContent = 'Klik untuk mengatur batas'; left\.textContent = 'Belum Diatur'; prog\.style\.width = '0%'; return; \}.*?prog\.style\.width = `\$\{pct\}%`; prog\.className = 'progress-bar-fill ' \+ \(pct >= 90 \? 'bg-danger' : \(pct >= 70 \? 'bg-warning' : 'bg-primary'\)\);", lambda m: budget_logic, js, flags=re.DOTALL)
