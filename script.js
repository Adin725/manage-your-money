const App = (() => {
    ...
    const UI = {
        ...
        fullscreenChart(canvasId) {
            const origChart = charts[canvasId];
            if (!origChart) return;
            const bg = document.body.classList.contains('dark-theme') ? '#1E293B' : '#FFFFFF';
            const color = document.body.classList.contains('dark-theme') ? '#F8FAFC' : '#1E293B';
            ...
            Swal.fire({
                html: `<div style="width:100%; height:75vh; position: relative;"><canvas id="fs-${canvasId}"></canvas></div>`,
                ...
                color: color,
                didOpen: () => {
                    const ctx = document.getElementById(`fs-${canvasId}`);
                    if (ctx) {
                        new Chart(ctx, {
                            type: origChart.config.type,
                            data: newData,
                            options: Object.assign({}, origChart.config.options, { maintainAspectRatio: false, responsive: true })
                        });
                    }
                }
            });
                    ...
                    ${Object.keys(cat).sort((a,b) => cat[b] - cat[a]).map(k => `
                        <div class="activity-item mb-2" style="border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 12px; background: var(--surface);">
                            <div class="activity-left">
                                <div class="activity-icon expense" style="background: rgba(59, 130, 246, 0.1); color: var(--primary); border-radius: var(--radius-sm);"><i class="ph-bold ph-tag"></i></div>
            ...
            if (budget > 0) {
                if (finalProjection > budget) {
                    ...
                    <div class="insight-card-modern" style="position: relative; overflow: hidden; border: 1px solid var(--border-color); background: var(--surface); align-items: flex-start; padding: 24px; border-radius: var(--radius-md); box-shadow: var(--shadow-sm); display: flex; flex-direction: column;">
                        <div class="circle circle-1 floating-slow" style="position: absolute; top: -50px; right: -50px; width: 150px; height: 150px; border: 2px solid rgba(59, 130, 246, 0.1); border-radius: 50%;"></div>
                        <div class="circle circle-2 floating-fast" style="position: absolute; bottom: -30px; left: -30px; width: 100px; height: 100px; border: 2px solid rgba(59, 130, 246, 0.1); border-radius: 50%;"></div>
                        
                        <div style="z-index: 1; display: flex; justify-content: space-between; align-items: center; width: 100%; margin-bottom: 12px;">
                            <span style="font-size: 0.95rem; opacity: 0.9; color: var(--text-main);">Peringatan AI</span>
                            <i class="ph-fill ph-warning" style="font-size: 1.5rem; color: var(--danger);"></i>
                        </div>
                        <h2 class="text-danger" style="z-index: 1; font-size: 2.2rem; font-weight: 800; margin-bottom: 8px; text-transform: uppercase;">OVERBUDGET!</h2>
                        <p style="z-index: 1; font-size: 0.95rem; color: var(--text-main); opacity: 0.9; line-height: 1.5; margin-bottom: 12px;">Kecepatan jajanmu <strong class="text-danger">${formatRp(dailyAvg).replace(',00','')} / hari</strong>. Di akhir bulan kamu akan tembus <strong class="text-danger">${formatRp(finalProjection).replace(',00','')}</strong>.</p>
                        <div style="z-index: 1; background: var(--bg-color); padding: 10px 14px; border-radius: var(--radius-sm); font-size: 0.85rem; font-weight: 600; width: 100%; color: var(--text-main);"><span class="text-danger">Saran:</span> Batasi maks ${formatRp((budget-totalSpent)/remainingDays).replace('Rp','').replace(',00','')}/hari mulai besok.</div>
                    </div>`;
                } else {
                    insightHtml = `
                    <div class="insight-card-modern" style="position: relative; overflow: hidden; border: 1px solid var(--border-color); background: var(--surface); align-items: flex-start; padding: 24px; border-radius: var(--radius-md); box-shadow: var(--shadow-sm); display: flex; flex-direction: column;">
                        <div class="circle circle-1 floating-slow" style="position: absolute; top: -50px; right: -50px; width: 150px; height: 150px; border: 2px solid rgba(16, 185, 129, 0.1); border-radius: 50%;"></div>
                        <div class="circle circle-2 floating-fast" style="position: absolute; bottom: -30px; left: -30px; width: 100px; height: 100px; border: 2px solid rgba(16, 185, 129, 0.1); border-radius: 50%;"></div>
                        
                        <div style="z-index: 1; display: flex; justify-content: space-between; align-items: center; width: 100%; margin-bottom: 12px;">
                            <span style="font-size: 0.95rem; opacity: 0.9; color: var(--text-main);">Sistem Analisis</span>
                            <i class="ph-fill ph-robot" style="font-size: 1.5rem; color: var(--success);"></i>
                        </div>
                        <h2 class="text-success" style="z-index: 1; font-size: 2.2rem; font-weight: 800; margin-bottom: 8px; text-transform: uppercase;">JALUR AMAN</h2>
                        <p style="z-index: 1; font-size: 0.95rem; color: var(--text-main); opacity: 0.9; line-height: 1.5;">Kecepatan jajan terkendali. Sisa prediksi <strong class="text-success">Rp ${formatRp(budget - finalProjection).replace('Rp','').replace(',00','')}</strong></p>
                    </div>`;
                }
            } else {
                insightHtml = `
